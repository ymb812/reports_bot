...
from core.states.admin_menu import AdminStateGroup
...
async def admin_menu(message: types.Message, bot: Bot, state: FSMContext, dialog_manager: DialogManager):
    user = await User.get(user_id=message.from_user.id)
    if user.status != 'admin':
        return

    # send admin menu
    await dialog_manager.start(state=AdminStateGroup.menu, mode=StartMode.RESET_STACK)
